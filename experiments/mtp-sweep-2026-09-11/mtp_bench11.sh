...
PHASES=${PHASES:-"A00 A10 A01 A11"}
want() { case " $PHASES " in *" $1 "*) return 0 ;; *) return 1 ;; esac; }

log "sweep11 start: 2x2 capacity additivity | PHASES=$PHASES"
seed_audit
...
# ---------- P29: sweep11 — do the two capacity sources add up? (2x2) ----------
# PRE-REGISTERED BEFORE ANY READING. The registration text is the README's
# "sweep11 预注册" section, committed as c88568d (先落字后见数).
#
# WHY a 2x2 and not just the combined arm: CGS (+9,285 tokens, -1.66% @48t) and
# GMU 0.95 (+14,200 tokens, +0.05% @48t) were each measured ALONE and in
# DIFFERENT windows. Nobody has ever given both. The question that matters is
# not "what is the best config" but "are these two sources actually two" -- and
# that cannot be answered without a same-window baseline, which is what A00 is.
#
# All four arms boot on the SAME pair (2,3) inside the SAME window, so every
# comparison in this phase is same-pair and same-window by construction: the
# <=2.2% B1c pair-effect caveat does NOT apply to any claim this phase makes by
# itself. It applies only to links drawn against sweep9/10, which are recorded
# as indicative.

CGS_SIZES="1 2 4 8 16 24 32 40 48 56 64 72 80 88 96 104 112 120 128"
# SP1/SP2 = the pair every arm in this phase runs on. Pinned to 2+3 so A00 is
# directly comparable to sweep9's P9pair23 and sweep10's P10cgs.
SP1=${SP1:-2}
SP2=${SP2:-3}
SPEC_R2C="--speculative-config '$SPEC1'"

# cfg_metrics is defined in sweep9's phase block, NOT in bench7's top level.
# bench10's block called it anyway; it was undefined, the script had no `set -e`,
# so the run simply produced no cachecfg_*.txt and nothing said so. Redefined
# here so this phase cannot inherit the same hole. (坑 27's lesson generalised:
# a block must define or inherit EVERY symbol it names, and bash -n won't say.)
cfg_metrics() { # $1 tag $2... ports
  local tag=$1; shift
  for p in "$@"; do
    curl -s "http://127.0.0.1:$p/metrics" 2>/dev/null \
      | grep -aE "cache_config_info" >> $OUT/cachecfg_${tag}.txt
  done
}

# faildump — if an arm does not come up, the last 40 lines of both boot logs are
# kept. GMU alone already pushed KV usage to 99.5%; stacking CGS's +9,285 tokens
# on top is expected to sit near 99.9%, and "it would not boot" is a legitimate
# result here. It must be evidenced, not summarised as "failed".
faildump() { # $1 pfx
  local pfx=$1
  { echo "===== faildump $pfx $(date -u +%FT%TZ) ====="
    for i in 1 2; do
      echo "----- ${pfx}_$i tail -----"
      tail -n 40 $OUT/server_${pfx}_${i}.log 2>/dev/null
    done
  } >> $OUT/fail_${pfx}.txt
}

if want A00; then
  # Same-window baseline. Without this arm criterion B (additivity) cannot be
  # evaluated at all, because sweep9's and sweep10's baselines are other windows.
  pfx=P11a00
  log "A00 $pfx R2c baseline on ($SP1,$SP2), same window"
  boot_replica 8341 $SP1 $OUT/server_${pfx}_1.log "$SPEC_R2C"; A=$LAST_PID
  boot_replica 8342 $SP2 $OUT/server_${pfx}_2.log "$SPEC_R2C"; B=$LAST_PID
  if wait_up 8341 && wait_up 8342; then
    log "$pfx pair up ($A/$B)"
    startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
...
if want A10; then
  # CGS alone, re-measured in this window. Its only knob is the capture table:
  # keep the fine grid to 128, drop the 35 buckets above 136 that this line's
  # 24-60 lanes/replica can never reach.
  pfx=P11a10
  log "A10 $pfx R2c + CGS on ($SP1,$SP2)"
...
if want A01; then
  # GMU 0.95 alone, re-measured in this window. 0.95 of 44.52 GiB vs the 0.92
  # default (config/cache.py:69) that every arm in this project has taken
  # silently -- it was never named until sweep9.
  pfx=P11a01
  log "A01 $pfx R2c + GMU 0.95 on ($SP1,$SP2)"
  boot_replica 8341 $SP1 $OUT/server_${pfx}_1.log "$SPEC_R2C --gpu-memory-utilization 0.95"; A=$LAST_PID
  boot_replica 8342 $SP2 $OUT/server_${pfx}_2.log "$SPEC_R2C --gpu-memory-utilization 0.95"; B=$LAST_PID
  if wait_up 8341 && wait_up 8342; then
    log "$pfx pair up ($A/$B)"
    startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
    cfg_metrics ${pfx} 8341 8342
...
if want A11; then
  # THE SHIP CONFIG CANDIDATE: both capacity sources at once. Run LAST, so that
  # if it is the arm that OOMs at boot, the other three are already on disk.
  pfx=P11a11
  log "A11 $pfx R2c + CGS + GMU 0.95 on ($SP1,$SP2) -- the combined arm"
  boot_replica 8341 $SP1 $OUT/server_${pfx}_1.log "$SPEC_R2C --cudagraph-capture-sizes $CGS_SIZES --gpu-memory-utilization 0.95"; A=$LAST_PID
  boot_replica 8342 $SP2 $OUT/server_${pfx}_2.log "$SPEC_R2C --cudagraph-capture-sizes $CGS_SIZES --gpu-memory-utilization 0.95"; B=$LAST_PID
  if wait_up 8341 && wait_up 8342; then
    log "$pfx pair up ($A/$B)"
    startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
    cfg_metrics ${pfx} 8341 8342
    ...
    log "ABORT $pfx boot failed -- this is itself the answer, kept in fail_${pfx}.txt"
    faildump ${pfx}
  fi
  kill_srv $A; kill_srv $B
  log "$pfx phase done"
...
out = sys.argv[1]
ARMS = [("P11a00", "A00 baseline"), ("P11a10", "A10 CGS"),
        ("P11a01", "A01 GMU 0.95"), ("P11a11", "A11 CGS+GMU")]
RUNGS = [("c48t", 24), ("c120t", 60)]
...
def steady(pfx):
    """Steady-state Running and KV usage = the MODE over the whole server log.
    坑 26: engine_stats_*_s3.txt's last sample catches the drain edge, and a
    single reading is not a steady state either. The mode over the full log is
    both, mechanically."""
    f = f"{out}/server_{pfx}_1.log"
    if not os.path.exists(f):
        return None, None
    rs = re.findall(r"Running:\s*(\d+)\s*reqs,\s*Waiting:\s*(\d+)\s*reqs, "
                    r"GPU KV cache usage:\s*([0-9.]+)%", open(f, errors="replace").read())
    if not rs:
        return None, None
    return Counter(int(a) for a, b, c in rs).most_common(1)[0][0], \
           Counter(c for a, b, c in rs).most_common(1)[0][0]

def pnum(pfx):
    v = pool(pfx).replace(",", "")
    return int(v) if v.isdigit() else None

print("== sweep11: 2x2 -- do CGS and GMU add up? ==")
print("   all four arms: pair (2,3), same window, FORCE_SEED=0, one discarded w0")
base48 = total("P11a00", "c48t")
base120 = total("P11a00", "c120t")
print(f"   {'arm':14s} {'pool':>9s} {'Running':>8s} {'KV%':>7s} {'graphGiB':>9s} {'buckets':>8s} "
      f"{'@48t':>9s} {'d48':>7s} {'@120t':>9s} {'d120':>7s}")
for pfx, name in ARMS:
    run, kvu = steady(pfx)
    v48, v120 = total(pfx, "c48t"), total(pfx, "c120t")
    d48 = f"{(v48-base48)/base48*100:+6.2f}%" if (v48 == v48 and base48 == base48) else "   n/a"
    d120 = f"{(v120-base120)/base120*100:+6.2f}%" if (v120 == v120 and base120 == base120) else "   n/a"
    s48 = f"{v48:9.2f}" if v48 == v48 else "      nan"
    s120 = f"{v120:9.2f}" if v120 == v120 else "      nan"
    print(f"   {name:14s} {pool(pfx):>9s} {str(run) if run is not None else '-':>8s} "
          f"{str(kvu) if kvu is not None else '-':>7s} {graphmem(pfx):>9s} {buckets(pfx):>8s} "
          f"{s48} {d48} {s120} {d120}")

p00, p10, p01, p11 = pnum("P11a00"), pnum("P11a10"), pnum("P11a01"), pnum("P11a11")
print("-- pre-registered criteria --")
if None in (p00, p10, p01, p11):
    print("   pool missing on some arm -> criteria n/a (see the pool column)")
else:
    hi = max(p10, p01)
    print(f"   pools: A00 {p00:,}  A10 {p10:,} (+{p10-p00:,})  A01 {p01:,} (+{p01-p00:,})  "
          f"A11 {p11:,} (+{p11-p00:,} vs A00)")
    print(f"   A (direction): pool(A11) > max(A10,A01)={hi:,} ..... "
          f"{'HOLDS' if p11 > hi else 'FIRED -> the two sources are NOT two; the +4,733 token per +0.01 gmu conversion and 5.2 have to be rewritten'}")
    print(f"   B (magnitude): pool(A11) in [113,896, 120,942] (registered band, "
          f"= 93,934 + 23,485 +/-15%) ..... "
          f"{'HOLDS' if 113896 <= p11 <= 120942 else ('BELOW -> SUB-ADDITIVE, record the interaction' if p11 < 113896 else 'ABOVE -> SUPER-ADDITIVE, a third effect exists, go find it')}")
    inc10, inc01 = p10 - p00, p01 - p00
    ratio = (p11 - p00) / (inc10 + inc01) if (inc10 + inc01) else float("nan")
    print(f"   B cross-check (descriptive, NOT the criterion): same-window interaction "
          f"ratio = {ratio:.3f} (1.000 = perfectly additive); "
          f"interaction term A11-A10-A01+A00 = {p11-p10-p01+p00:+,} tokens")
print("-- throughput: does the price stack? --")
if base48 == base48 and total("P11a11", "c48t") == total("P11a11", "c48t") and base48:
    d = (total("P11a11", "c48t") - base48) / base48 * 100
    print(f"   C1: A11 @48t vs A00 = {d:+.2f}%  (centre prediction -1.61%) ..... "
          f"{'HOLDS' if d >= -2.5 else 'FIRED -> the two below-the-wall prices multiply; the spoken line needs the caveat'}")
else:
    print("   C1: n/a (incomplete)")
v10, v01, v11 = total("P11a10", "c120t"), total("P11a01", "c120t"), total("P11a11", "c120t")
if v10 == v10 and v01 == v01 and v11 == v11:
    m = max(v10, v01)
    print(f"   C2: A11 @120t {v11:.2f} >= max(A10 {v10:.2f}, A01 {v01:.2f}) = {m:.2f} ..... "
          f"{'HOLDS' if v11 >= m else 'FIRED -> capacity is not the only term ordering @120t'}")
else:
    print("   C2: n/a (incomplete)")
run11, kvu11 = steady("P11a11")
print(f"   descriptive: Running(A11) = {run11 if run11 is not None else '-'} "
      f"(registered [28, 31]); KV usage {kvu11 if kvu11 is not None else '-'}%")
print("   NOTE: 'Running' above is the MODE over the whole server log, not the last")
print("   engine snapshot -- see 坑 26. Cross-check the s3 column by hand if they differ.")

print("-- the ship-config sentence this phase is for --")
if p11 is not None and v11 == v11 and base48 == base48:
    print(f"   A11 = R2c + CGS + GMU 0.95: pool {p11:,} tokens, Running {run11}, "
          f"@48t {total('P11a11','c48t'):.2f} ({(total('P11a11','c48t')-base48)/base48*100:+.2f}% vs same-window baseline), "
          f"@120t {v11:.2f} ({(v11-base120)/base120*100:+.2f}% vs same-window baseline)")
    print(f"   historical anchor for the same config WITHOUT the two knobs: 652.98 @48t")
else:
    print("   incomplete -- do NOT write a ship-config sentence off this run")
EOF
