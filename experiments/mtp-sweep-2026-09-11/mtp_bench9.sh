...
PHASES=${PHASES:-"PAIR EG GMU MB"}
want() { case " $PHASES " in *" $1 "*) return 0 ;; *) return 1 ;; esac; }

log "sweep9 start: card pair / enforce-eager / gmu / mamba block size | PHASES=$PHASES"
seed_audit
...
# ---------- P27: sweep9 — card pair / enforce-eager / gmu / mamba block size ----------
# PRE-REGISTERED BEFORE ANY READING. The registration text is the README's
# "sweep9 预注册" section, committed as ced6204 (先落字后见数).
#
# The two source-level openings were already settled by reading, no GPU needed:
#   opening 1 (GDN state in fp32?)  -> DEAD: mamba_cache_dtype="auto" resolves to
#     model_dtype=bf16, and MambaDType admits only auto/float32/float16/bfloat16.
#   opening 6 (AWQ escapes Marlin?) -> DEAD: SM89 >= 75 and group 128 is in
#     MARLIN_SUPPORTED_GROUP_SIZES, so AWQ is auto-upgraded to awq_marlin.
# What is left is what this phase runs.

# cfg_metrics — block_size is what the whole capacity story turns on, and the
# engine reports it itself on /metrics as vllm:cache_config_info{...}, built from
# CacheConfig.metrics_info() = str() of every __dict__ entry. Captured live on
# every arm rather than inferred, and captured raw so a wrong regex here cannot
# destroy the evidence -- the raw file is kept either way.
...
# SP1/SP2 = the pair the single-pair phases run on. Pinned to 2+3 so each of them
# is directly comparable to sweep8's Z8 baseline, which ran on the same pair.
SP1=${SP1:-2}
SP2=${SP2:-3}
SPEC_R2C="--speculative-config '$SPEC1'"

if want PAIR; then
  # The terminal config, same window, two different card pairs. This is the one
  # phase where running the two arms in different windows would destroy the
  # measurement, so both pairs are booted back to back inside this phase.
  # Section 7's layout factor compares an arm on (1,2) against one on (2,3); if
  # the pair itself is worth more than B1c's 2.21%, that factor has to be redone.
  for pair in "1 2" "2 3"; do
    PA=${pair% *}; PB=${pair#* }; pfx=P9pair${PA}${PB}
    log "PAIR $pfx booting on ($PA,$PB)"
    boot_replica 8341 $PA $OUT/server_${pfx}_1.log "$SPEC_R2C"; A=$LAST_PID
    boot_replica 8342 $PB $OUT/server_${pfx}_2.log "$SPEC_R2C"; B=$LAST_PID
    if wait_up 8341 && wait_up 8342; then
      log "$pfx pair up ($A/$B)"
      startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
      ...
      spec_metrics ${pfx}_c120t 8341 8342
    else
      log "ABORT $pfx boot failed"
    fi
...
if want EG; then
  # --enforce-eager hands the CUDA graph pool back to KV. The point is NOT to
  # adopt it -- it is to price how many lanes that pool is worth. Throughput is
  # expected to FALL (host launch overhead) while Running is expected to RISE.
  # If Running does not move, the graph pool was never carved out of the KV pool
  # and the mechanism this whole section rests on has to be rewritten.
  pfx=P9eg
  log "EG $pfx booting on ($SP1,$SP2) with --enforce-eager"
  boot_replica 8341 $SP1 $OUT/server_${pfx}_1.log "$SPEC_R2C --enforce-eager"; A=$LAST_PID
  boot_replica 8342 $SP2 $OUT/server_${pfx}_2.log "$SPEC_R2C --enforce-eager"; B=$LAST_PID
  if wait_up 8341 && wait_up 8342; then
    log "$pfx pair up ($A/$B)"
    startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
    cfg_metrics ${pfx} 8341 8342
...
if want GMU; then
  # The baseline runs at vLLM 0.28's default 0.92 (config/cache.py:69), 40.96 GiB
  # of the 44.52 GiB the driver actually shows. 0.95 is +0.03 = +1.34 GiB, which
  # at ~192 KiB a block is roughly +7 blocks, roughly +3 lanes/replica. Never
  # swept -- every run so far took the default without ever naming it.
  pfx=P9gmu
  log "GMU $pfx booting on ($SP1,$SP2) with --gpu-memory-utilization 0.95"
...
if want MB; then
  # Pure probe, no directional prediction. --mamba-block-size sets the mamba page
  # granularity; the capacity story says the KV block size is derived from that
  # page, and that the per-request snapshot tax is paid IN BLOCKS. If this knob
  # moves block_size, the unit the tax is levied in is itself a lever. One point,
  # and the only question asked is whether anything moved at all.
  pfx=P9mb
  log "MB $pfx booting on ($SP1,$SP2) with --mamba-block-size 32"
  boot_replica 8341 $SP1 $OUT/server_${pfx}_1.log "$SPEC_R2C --mamba-block-size 32"; A=$LAST_PID
  boot_replica 8342 $SP2 $OUT/server_${pfx}_2.log "$SPEC_R2C --mamba-block-size 32"; B=$LAST_PID
  if wait_up 8341 && wait_up 8342; then
    log "$pfx pair up ($A/$B)"
    startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
    cfg_metrics ${pfx} 8341 8342
    FORCE_SEED=0 bench_pair ${pfx}_w0 60 120 8341 2 $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
    FORCE_SEED=0 bench_pair ${pfx}_c48t 24 48 8341 2 $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
  else
    log "ABORT $pfx boot failed"
  fi
  kill_srv $A; kill_srv $B
...
def cfg(tag):
    """block_size / mamba_block_size as the engine itself reports them.
    \b keeps block_size from matching inside mamba_block_size (_ is a word char,
    so there is no boundary between the two)."""
    fs = glob.glob(f"{out}/cachecfg_{tag}.txt")
    if not fs:
        return "-", "-"
    txt = open(fs[0]).read()
    def grab(k):
        m = re.findall(r"\b" + k + r"[\"']?\s*[:=]\s*[\"']?(\d+)", txt)
        return m[-1] if m else "-"
    return grab("block_size"), grab("mamba_block_size")

def kvsize(tag):
    """GPU KV cache size in tokens, from that arm's boot log."""
    f = glob.glob(f"{out}/server_{tag}_1.log")
    if not f:
        return "-"
    m = re.findall(r"GPU KV cache size:\s*([0-9,]+) tokens", open(f[0]).read())
    return m[-1] if m else "-"

print("== sweep9 ==")
print("-- PAIR: terminal config, one window, two card pairs --")
pa, pb = total("P9pair12", "c48t"), total("P9pair23", "c48t")
qa, qb = total("P9pair12", "c120t"), total("P9pair23", "c120t")
for rung, a, b in (("c48t", pa, pb), ("c120t", qa, qb)):
    if a != a or b != b:
        print(f"   @{rung}: n/a (incomplete)")
        continue
    d = (b - a) / a * 100
    print(f"   @{rung}: (1,2) {a:8.2f}   (2,3) {b:8.2f}   -> {d:+6.2f}%")
    print(f"           criterion |d| < 2.0%: {'HOLDS' if abs(d) < 2.0 else 'FIRED -> the layout +6.57% has to be redone'}")
for tag in ("P9pair12", "P9pair23"):
    for rung in ("c48t", "c120t"):
        e = eng(tag, rung)
        bs, mbs = cfg(tag)
        print(f"   {tag} @{rung}: Running/Waiting = {e if e else 'MISSING'}  "
              f"block_size={bs} mamba_block_size={mbs}  pool={kvsize(tag)}")

print("-- EG / GMU vs the same-pair baseline (all on 2+3, this window) --")
base48, base120 = total("P9pair23", "c48t"), total("P9pair23", "c120t")
for pfx, name in (("P9eg", "--enforce-eager"), ("P9gmu", "--gpu-memory-utilization 0.95")):
    for rung, base in (("c48t", base48), ("c120t", base120)):
        v = total(pfx, rung)
        e = eng(pfx, rung)
        bs, mbs = cfg(pfx)
        if v != v:
            print(f"   {name:34s} @{rung}: n/a (incomplete)")
            continue
        d = (v - base) / base * 100 if base == base else float('nan')
        print(f"   {name:34s} @{rung}: {v:8.2f}  ({d:+6.2f}% vs baseline)  "
              f"Running/Waiting = {e if e else 'MISSING'}  block_size={bs} pool={kvsize(pfx)}")
print("   baseline = P9pair23, the same config on the same pair, booted in this window.")
print("   EG criterion: Running must RISE above 24. If it does not, the graph pool")
print("   was never carved out of the KV pool and the mechanism is wrong.")
print("   GMU criterion: Running 27+/-2. If it does not move, or if throughput falls")
print("   by more than 1%, the prediction fails as written.")

print("-- MB: pure probe, one question --")
bs_b, mbs_b = cfg("P9pair23")
bs_m, mbs_m = cfg("P9mb")
v48 = total("P9mb", "c48t")
moved = (bs_m not in ("-",)) and (bs_b not in ("-",)) and (bs_m != bs_b)
print(f"   block_size      : baseline {bs_b}  vs  --mamba-block-size 32 {bs_m}")
print(f"   mamba_block_size: baseline {mbs_b}  vs  --mamba-block-size 32 {mbs_m}")
print(f"   KV pool         : baseline {kvsize('P9pair23')}  vs  --mamba-block-size 32 {kvsize('P9mb')}")
if v48 == v48:
    print(f"   @48t throughput : {v48:8.2f}  (baseline {base48:.2f}, {(v48-base48)/base48*100:+.2f}%)")
else:
    print("   @48t throughput : n/a (incomplete)")
print(f"   VERDICT: {'block_size MOVED -> a real lever, worth an arm' if moved else 'block_size UNCHANGED (or unreadable) -> dead on this model, record it and stop'}")
EOF
