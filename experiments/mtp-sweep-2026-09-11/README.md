...
**⚠️ 扫描扫出来的、预注册里没有的东西（必须标为事后观察，不得当作结论）**

**k=0 在 @138t 断崖：354.08/354.14 → 323.04/323.21，两个副本同时掉 −8.8%，
且两个副本互相吻合到 0.05%。** 这不是单副本抖动，是结构性事件。
同一窗口的引擎采样显示 `Running: 45 / Waiting: 15–18 / GPU KV cache usage: 98.4%`
（@132t 时是 `Running: 38 / Waiting: 8 / 83.1%`），符合"撞 KV 块墙"的签名；
@144t 部分回升到 657.45，但仍在 98.4% —— **所以墙能解释跌落、解释不了回升，
机制未闭合。**

**它的证据等级必须说清楚：这是细扫顺带发现的，不是预注册检验过的假设。**
按本项目的规矩，它现在的身份是**"生成假设"**，要进任何结论都必须先做一次
预注册复测（判据写死、含证伪条件）。**在此之前，它不得用于修改部署建议，
也不得写进对外数字。**
...
| `sweep7/bench_{R2bZx,R2cZx}_{w0,c120t,c126t,c132t,c132tr,c138t,c144t}_p{1,2}.log` | Z3 的交叉带细扫（28 个）——`@132tr` 是 @132t 的**就地复测**；**P1 证伪：k=1 凹口在 63 lanes/副本而非 66；另见 k=0 在 @138t 的 −8.8% 断崖（事后观察）** |
