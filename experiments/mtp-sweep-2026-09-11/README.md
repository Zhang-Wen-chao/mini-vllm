...
**第三格的机制已定位：MTP 接受率是「prompt 集」的函数。**
...
含义要写进方法论：**k=1 的臂天生比 k=0 多一个方差项**——它的吞吐依赖 prompt 集
决定的接受率，k=0 不吃这个。所以 k=1 的跨窗比较天然比 k=0 不可靠，
除非用同一个 seed（同一 prompt 集）。
...
| 量化 bf16 → fp8 权重+KV | B0 375.09 → B1 488.01 | **+30.1%** |
| 调度步长 2048 → 8192 | B1 488.01 → B1b 509.83 | **+4.5%** |
| 布局 TP2 → 2 副本（fp8, k=0） | B1b 509.83 → **R2b48 549.55** | **+7.79%** |
| 投机 k=0 → k=1（同布局同 lanes） | **R2b48 549.55** → R2c_c48t 600.40 | **+9.25%** |
| 合计 | 375.09 → 600.40 | **+60.07%** |

1.301 × 1.0447 × 1.0779 × 1.0925 = 1.60055 → 375.09 × 1.60055 = **600.35**，
实测 600.40，差 **0.008%**。**sweep6 那个「+66%」现在应改口为同窗的「+60.1%」**，
差的那 3.7% 就是 R2c@48t 的跨窗缺口（下面说明它多半是负载造成的）。
切分也从 (布局 +7.5% / 投机 +12.9%) 修正为 **(布局 +7.8% / 投机 +9.3%)**——
预注册写的「落 [532,564] → 布局 ≈+7.5%、投机 ≈+9.5%」两处都对上了。
...
652.98 > 618.76 > 600.40 —— **完全相反**；而 steps/s 排序 180.0 > 170.0 > 165.5
与吞吐**完全一致**。所以三者的差异在**解码步速**，不在接受率。
**接受率解释被证伪，撤回**——预注册里写死的证伪条件起了作用，不必争论。

**新的头号嫌疑：host loadavg（共享机的邻居）。** 我为这个目的加的 loadavg 记录：
