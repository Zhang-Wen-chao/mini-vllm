...
- **正在跑（15:1x 起，一晚）**：`R2cABr`（一 boot 内 0/702/0 三点，分离池/顺序/噪声）、
  `R2bZ`（k=0 阶梯 108/120/132/144t **同池**，夹逼 682.96）、
  `R2cZ`（k=1 @48t 三次重复 + c120t/c144t，全 seed 0 → k=1 噪声底）、
  `FV`（四因子链五节点全 seed 0 重跑）。因邻居占 GPU1/2/3，开跑即进 `wait_gpus` 门。
