...
**R2cZ 的预注册**（写于 R2cZ 尚未落盘时，17:0x）：
1. 历史交叉点在 60 与 72 并发/副本之间（k=1 @120t 输 666.06 < 682.96，@144t 赢 670.35 > 653.79）。
   现在 **k=0 在 66 并发/副本这一点被实测成 703.26，高于它两侧的 60 和 72**，说明这个池里
   k=0 的极大值就在 66 附近。
2. **预测：k=1 在 @132t 输**（`R2cZ_c132t < R2bZ_c132t`），交叉点上移到 (66, 72] 并发/副本。
3. **证伪条件**：若 `R2cZ_c132t > 703.26`，本预测作废——且「k=1 越靠近墙越占优」这个机制
   也一并作废，需要另找解释，不许事后改口成「本来就在 66 和 72 之间」。
4. **解释护栏（先写死）**：@132t 上 k=0 处在自己的极大值、k=1 可能已过峰。所以即使 k=0 赢，
   也不能写成「66 并发时 k=0 优于 k=1」而不注明**两条曲线形状不同**；要说的是
   「k=1 的峰更靠左」。
5. 噪声底（`R2cZ_c48t_{a,b,c}` 背靠背三次，同 boot 同池）：**预测 spread ≤ 1%**。
