...
log "sweep8 start: the k=2 high-concurrency arm | PHASES=$PHASES | RPA=$RPA RPB=$RPB"
seed_audit
...
# ---------- P26: Z8 — the k=2 high-concurrency arm (sweep8) ----------
# PRE-REGISTERED BEFORE ANY READING. The registration text is the README's
# "sweep8 预注册" section, committed as the parent of this file (先落字后见数).
#
# WHY: sweep7 bracketed the k=0/k=1 crossover to (66,72] lanes/replica, but
# k=2 has NO reading anywhere in the high-concurrency band. The whole dataset
# holds exactly two k=2 points: R2a (24 lanes/replica, k=2 LOSES 578.49 vs
# 652.98) and single-card @16 (k=2 WINS 301.4 vs 269.7). Everything between
# 24 lanes/replica and 72 has never been measured with k=2.
#
# DESIGN: both arms run on the SAME card pair (RPA/RPB) in the SAME window, one
# boot each, FORCE_SEED=0 single pool, one discarded warm-up point -- the Z2
# shape. Every comparison in this phase is therefore same-pair and same-window,
# so the <=2.2% B1c pair-effect caveat does NOT apply to any claim this phase
# makes by itself; it applies only to links drawn against sweep7's GPU1+2
# ladders, and no such link is load-bearing here.
#
# NOTE ON ESCAPING: SPEC2 is defined at top level with PLAIN quotes, exactly
# like SPEC1 on line 67 and unlike the RUN_EXTRA block's \" variant -- the
# comment on line 68 is explicit that a top-level assignment keeps the
# backslash literal. The plain-quote form is the one every replica arm in
# sweep7 actually booted with.
SPEC2='{"method": "mtp", "num_speculative_tokens": 2}'
if want Z8; then
  log "Z8 pair = RPA=$RPA RPB=$RPB (all Z8 comparisons are same-pair, same-window)"
  for spec in 1 2; do
    if [ "$spec" = "1" ]; then
      sp="--speculative-config '$SPEC1'"; pfx=R2cZ8; kname="k=1"
    else
      sp="--speculative-config '$SPEC2'"; pfx=R2c2Z8; kname="k=2"
    fi
    log "Z8 $pfx ($kname) booting on $RPA+$RPB"
    boot_replica 8341 $RPA $OUT/server_${pfx}_1.log "$sp"; Z81=$LAST_PID
    boot_replica 8342 $RPB $OUT/server_${pfx}_2.log "$sp"; Z82=$LAST_PID
    if wait_up 8341 && wait_up 8342; then
      log "$pfx pair up ($Z81/$Z82)"
      startup_lines $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
      # w0: discarded warm-up, same shape and pool as @120t, so w0 -> c120t is
      # a controlled position-1-vs-position-3 reading of the boot ramp itself,
      # on both arms. PITFALL 20 (a boot's first measured point runs ~1.5% slow)
      # is why it is discarded rather than reported.
      FORCE_SEED=0 bench_pair ${pfx}_w0 60 120 8341 2 $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
      # 24/32 straddle k=2's ~17-lane wall from BELOW (the only regime where the
      # registration predicts k=2 wins); 48/108/120/132/144 are at and above
      # k=1's 24-lane wall, where it predicts k=2 loses.
      for c in 24 32 48 108 120 132 144; do
        FORCE_SEED=0 bench_pair ${pfx}_c${c}t $((c/2)) $c 8341 2 $OUT/server_${pfx}_1.log $OUT/server_${pfx}_2.log
      done
      spec_metrics ${pfx}_c132t 8341 8342
    else
...
import os, re, sys
out = sys.argv[1]
RUNGS = [("w0", 60), ("c24t", 12), ("c32t", 16), ("c48t", 24),
         ("c108t", 54), ("c120t", 60), ("c132t", 66), ("c144t", 72)]
ARMS = [("R2cZ8", "k=1"), ("R2c2Z8", "k=2")]
...
def eng(pfx, rung):
    """Last Running/Waiting sample of the point's 3rd engine snapshot."""
...
def kv(pfx, rung):
    ...
    if not os.path.exists(f):
        return None
    m = re.findall(r"GPU KV cache usage:\s*([0-9.]+)%", open(f).read())
    return m[-1] if m else None

print("== Z8: k=2 at high concurrency -- the band that was never measured ==")
print("   pair: read the 'Z8 pair = ...' line at the top of this run's log.")
print("   All comparisons below are same-pair, same-window by construction.")
missing = [f"{p}_{r}" for p, _ in ARMS for r, _ in RUNGS if total(p, r) != total(p, r)]
print(f"   completeness: {'ALL SIXTEEN PRESENT' if not missing else 'INCOMPLETE -> ' + ', '.join(missing)}")
for pfx, name in ARMS:
    print(f"   {name} ({pfx}):")
    for rung, lanes in RUNGS:
        v = total(pfx, rung)
        e = eng(pfx, rung)
        k = kv(pfx, rung)
        es = f"Running {e[0]:>3} Waiting {e[1]:>3}" if e else "engine state MISSING"
        ks = f"KV {k}%" if k else "KV n/a"
        vs = f"{v:8.2f}" if v == v else "     nan"
        print(f"      @{rung:<6} ({lanes:>2} lanes/rep)  {vs}   {es}   {ks}")
print("== head to head: k=2 vs k=1, point for point ==")
verdict_rows = []
for rung, lanes in RUNGS:
    a, b = total("R2cZ8", rung), total("R2c2Z8", rung)
    if a != a or b != b:
        print(f"   @{rung:<6} ({lanes:>2} lanes/rep): n/a (incomplete)")
        continue
    d = (b - a) / a * 100
    who = "k=2" if b > a else "k=1"
    verdict_rows.append((rung, lanes, a, b, d))
    print(f"   @{rung:<6} ({lanes:>2} lanes/rep): k=1 {a:8.2f}  k=2 {b:8.2f}  -> {d:+7.2f}%  ({who} wins)")
print("== pre-registered criteria ==")
d = {r: (b - a) / a * 100 for r, l, a, b, _ in verdict_rows}
c = {r: (a, b) for r, l, a, b, _ in verdict_rows}
ok_a = all(r in d and d[r] > 0 for r in ("c24t", "c32t"))
if "c132t" in d:
    ok_b = d["c132t"] < 0 and d["c132t"] <= -10.0
else:
    ok_b = False
print(f"   (a) k=2 wins at BOTH @24t and @32t .......... {'HOLDS' if ok_a else 'FAILS'}")
for r in ("c24t", "c32t"):
    if r in d:
        print(f"         @{r}: {d[r]:+.2f}%")
print(f"   (b) k=2 loses @132t by >= 10% .............. {'HOLDS' if ok_b else 'FAILS'}")
if "c132t" in d:
    print(f"         @132t: {d['c132t']:+.2f}%")
print(f"   CONJUNCTION: {'BOTH HOLD -> the spoken line stands' if (ok_a and ok_b) else 'NOT MET -> see the falsification clause in the registration'}")
hi = [(r, d[r]) for r in ("c48t", "c108t", "c120t", "c132t", "c144t") if r in d]
broken = [(r, v) for r, v in hi if v >= 0]
print(f"   FALSIFIER (any k=2 >= k=1 at @48t and above): {'FIRED -> ' + ', '.join(f'{r} {v:+.2f}%' for r, v in broken) if broken else 'not fired'}")
k1r = [total("R2cZ8", r) for r in ("c108t", "c120t", "c132t", "c144t")]
k1r = [v for v in k1r if v == v]
if len(k1r) == 4:
    sp = (max(k1r) - min(k1r)) / min(k1r) * 100
    print(f"   (3) k=1 high rungs spread (108/120/132/144): {sp:.2f}%  "
          f"-> {'HOLDS (<3%, they are one wall)' if sp < 3 else 'FAILS (>=3%, not one operating point)'}")
if "c132t" in c:
    a, b = c["c132t"]
    print(f"   x-pair link to sweep7's Z2 k=1 @132t = 654.76 (GPU1+2): "
          f"{a:.2f} = {(a-654.76)/654.76*100:+.2f}%  (<=2.2% pair caveat, recorded not asserted)")
print("   NOTE: 'k=1 pins at 24 lanes/replica' is an ENGINE-STATE claim -- read it off")
print("   the Running column above, not off the throughput. Same for k=2's ~17.")
EOF
fi
